<template>
  <div v-if="isOpen" class="modal-overlay" @click="closeModal">
    <div class="modal-content" @click.stop>
      ...
      <!-- 이메일 로그인 폼 -->
      <div v-if="isEmailLogin" class="modal-form email-login-form">
        <input v-model="email" type="email" placeholder="이메일" required />
        <input v-model="password" type="password" placeholder="비밀번호" required />

        <button class="submit-btn" @click="submitEmailLogin" :disabled="isLoading">
          {{ isLoading ? '로그인 중...' : '로그인' }}
        </button>
      </div>
      ...
      <div v-else class="modal-form">
        ...
        <div class="social-login">
          ...
          <button class="social-btn" @click="isEmailLogin = true" :disabled="isLoading">
            <svg class="social-icon" viewBox="0 0 24 24">
              <path fill="#1A1A1A"
                d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" />
            </svg>
            {{ isLoading ? '가입 중...' : '이메일로 로그인' }}
          </button>
        </div>
...
const submitEmailLogin = async () => {
  ...
  try {
    ...
    alert('로그인 성공!')
    setAuth(res.data.token) // 인증 상태 업데이트
    closeModal()
